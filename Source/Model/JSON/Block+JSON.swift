...
extension Block {
  ...
  */
  ...
  /**
   ...
   If it isn't, this method simply returns back the original value.
  ...
  internal class func translatedMessage(_ message: String) -> String {
    ...
    let matches = TranslationMatcher.shared.matches(
    ...
    while i < translatedMessage.endIndex {
      ...
      case .percentFound:
        if let number = Int(String(character)) {
          // Number found and we are supposed to be parsing number tokens
      ...
      case .percentAndDigitFound:
        // NOTE: This state is only accessible if we are parsing number tokens
        if let number = Int(String(character)) {
          // Multi-digit number.
...
/**
 Helper class for storing regular expression used for translating key values.
 */
fileprivate class TranslationMatcher: NSRegularExpression {
  // Shared instance.
  fileprivate static var shared = TranslationMatcher()
